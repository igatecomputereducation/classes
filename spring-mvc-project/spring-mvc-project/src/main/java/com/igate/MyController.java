...
public class MyController {
	...
	public ModelAndView products_updateproducts(@PathVariable("pcode")int pcode) {
		ModelAndView m=new ModelAndView();
		Products p=daoimpl.getProduct(pcode);
		m.addObject("det",p);
		m.setViewName("updatepro_form");
		return m;
	}
	
	@RequestMapping(path="/modifyproduct",method=RequestMethod.POST)
	public String products_modifyproducts(@RequestParam("pcode")int pcode,@RequestParam("pname")String pname,@RequestParam("price")float price,@RequestParam("stock")int stock) {
		String qry=String.format("update products set pname='%s',price=%f,stock=%d where pcode=%d", pname,price,stock,pcode);
